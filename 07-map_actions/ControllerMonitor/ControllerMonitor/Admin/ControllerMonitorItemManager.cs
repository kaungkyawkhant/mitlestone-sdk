...
namespace ControllerMonitor.Admin
{
    ...
    /// </summary>
    public class ControllerMonitorItemManager : ItemManager
    {
        private ControllerMonitorUserControl _userControl;
        private Guid _kind;

        // Definitions for Event Server
        internal static Guid EventGroupId = new Guid("A1940D98-4DDA-42F7-8815-C91C0EE87010");
        internal EventGroup eventGroup = new EventGroup() { ID = EventGroupId, Name = "Controller Monitor Events" };

        private Guid IdCtrlUp = new Guid("5ECE9C6B-E5F4-40D6-BFDF-6A429BFAB8A2");
        private Guid IdCtrlDown = new Guid("740FC193-856C-4C3A-92B5-7F133D30FE21");
        ...
        public ControllerMonitorItemManager(ControllerMonitorDefinition definition, Guid kind) : base()
        {
        ...
        {
            _userControl = new ControllerMonitorUserControl();
        ...
        {
            return new ControllerMonitorAddUserControl();
        }
        ...
        public override bool ValidateAndSaveUserControl()
        {
            if (CurrentItem != null)
            {
                ...
                Configuration.Instance.SaveItemConfiguration(ControllerMonitorDefinition.ControllerMonitorPluginId, CurrentItem);
            }
            return true;
        }
        ...
        {
            CurrentItem = new Item(suggestedFQID, ((ControllerMonitorAddUserControl)addUserControl).ItemName);
            Configuration.Instance.SaveItemConfiguration(ControllerMonitorDefinition.ControllerMonitorPluginId, CurrentItem);
            return CurrentItem;
        }
        ...
        public override void DeleteItem(Item item)
        {
            if (item != null)
            {
                Configuration.Instance.DeleteItemConfiguration(ControllerMonitorDefinition.ControllerMonitorPluginId, item);
            }
        ...
        public override List<Item> GetItems()
        {
            ...
            List<Item> items = Configuration.Instance.GetItemConfigurations(ControllerMonitorDefinition.ControllerMonitorPluginId, null, _kind);
            return items;
        }
        ...
        {
            List<Item> items = Configuration.Instance.GetItemConfigurations(ControllerMonitorDefinition.ControllerMonitorPluginId, parentItem, _kind);
            return items;
        }
        ...
        {
            Item item = Configuration.Instance.GetItemConfiguration(ControllerMonitorDefinition.ControllerMonitorPluginId, _kind, fqid.ObjectId);
            return item;
        }
        ...
        {
            return new System.Collections.ObjectModel.Collection<VideoOS.Platform.Data.EventGroup>() { eventGroup };
        }
        ...
        public override System.Collections.ObjectModel.Collection<VideoOS.Platform.Data.EventType> GetKnownEventTypes(System.Globalization.CultureInfo culture)
        {
            return new System.Collections.ObjectModel.Collection<VideoOS.Platform.Data.EventType> {
                new EventType() {
                            ...
                            ID= IdCtrlUp,
                            DefaultSourceKind=ControllerMonitorDefinition.ControllerMonitorKind,
                            Message="ControllerUp",
                            SourceKinds=new List<Guid>(){ControllerMonitorDefinition.ControllerMonitorKind}},
                        new EventType() {
                            GroupID=EventGroupId,
                            ID= IdCtrlDown,
                            DefaultSourceKind=ControllerMonitorDefinition.ControllerMonitorKind,
                            Message="ControllerDown",
                            SourceKinds=new List<Guid>(){ControllerMonitorDefinition.ControllerMonitorKind}}
                       };
        ...
        private object TriggerReceiver(VideoOS.Platform.Messaging.Message message, FQID dest, FQID sender)
        {
            try
            {
                if (dest != null && dest.Kind == _kind)
                {
                    ...
                    {
                        if (command == ControllerMonitorDefinition.GateCloseCommand)
                        {
                            EnvironmentManager.Instance.Log(false, "ControllerMonitor", "ControllerMonitor- Sending GateClose command " + command, null);
                            if (userSID == null)
                        ...
                        if (command == ControllerMonitorDefinition.GateOpenCommand)
                        {
                            EnvironmentManager.Instance.Log(false, "ControllerMonitor", "ControllerMonitor- Sending GateOpen command " + command, null);
                            if (userSID == null)
                                SecurityAccess.CheckPermission(item, command);
                            else
                                SecurityAccess.CheckPermission(item, command, userSID);
            ...
            {
                EnvironmentManager.Instance.Log(false, "ControllerMonitor", "ControllerMonitor " + ex.Message, new[] { ex });
